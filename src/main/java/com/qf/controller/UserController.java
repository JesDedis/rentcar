...
public class UserController {
    ...
    public  User  updateTel(User user){
        ...
        if(userByTel==null){
            Integer integer = userServer.updateTel(user.getTel(), user1.getId());
    ...
    @ResponseBody
    public User updateEmail(String email,HttpServletRequest request){
        System.out.println("email");
        User user = (User) request.getSession().getAttribute("user");
        user.setEmail(email);
        Integer userByEmail = userServer.updateEmail(user);
        if(userByEmail != null){
            userServer.updateEmail(user);
    ...
    public  User  updatePWD(User user){
        Integer userByPWD = userServer.updatePWD(user.getPassword(),user.getId());
        if(userByPWD != null){
            Integer integer = userServer.updatePWD(user.getPassword(),user.getId());
